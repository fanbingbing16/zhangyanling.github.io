  ...
  <div>
    <el-form ref="form" :inline="true" :model="form" label-width="80px">
      <el-form-item label="过敏史">
        <el-input placeholder="请输入内容" prefix-icon="el-icon-search" v-model="form.allergiHistory"> </el-input>
      </el-form-item>
    ...
    <template>
      <div  v-for="item in searchData" class="card" :key="item.id">
...
export default {
  ...
  created() {
    ...
    if (!this.isManage) {
      ...
      this.$axios.get('http://localhost:3000/api/Manage/searchOrder').then(response => {
        console.log(response, 'response')
        if (response.status === 200) {
          this.data = response.data.filter(item => {
            item.sfz_number = String(item.sfz_number)
  ...
  methods: {
    search() {
      this.searchData = this.data.filter(item => {
        return this.form.allergiHistory.length > 0 ? item.allergiHistory && item.allergiHistory.includes(this.form.allergiHistory) : true && this.form.sex === item.sex
      })
    }
  },
  data() {
